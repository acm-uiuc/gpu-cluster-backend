...
class GPUContainerController(ContainerController):
    ...
    def create_container(self, image, user="", token_required=False, budget=-1, num_gpus=1):
        ...
        num_available_gpus = len(self.docker_client.list_gpus())
        if num_gpus > num_available_gpus:
            num_gpus = num_available_gpus

        gpus = []
        memory_usage = self.docker_client.gpu_memory_usage()
        for g in range(num_gpus):
            for gpu, used in memory_usage.items():
                #if used < memory_usage[gpu[-1]]:       causes code to crash, not sure what this is supposed to be
                gpus.append(gpu)
